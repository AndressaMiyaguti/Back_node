...
router.get("/car/:id", (req, res) => {  //Busca de carro por id
  ...
  const foundedCar = db.filter((currentCar) => { //Recebe o carro atual (currentCar)
...
router.patch("/car/edit/:id", (req, res) => {
  ...
  const foundedCar = db.filter((currentCar) => { //O filter vai trazer um array de objeto, onde queremos alterar um desses objetos
        return currentCar.id === id; 
  })
  ...
  db.forEach((currentCar, index) => { //O forech recebe o carro atual e o indice
    if  (currentCar.id === id){ // se o id do carro recebido for igual ao id do param
      db[index] = editedCar;
    }
  })
